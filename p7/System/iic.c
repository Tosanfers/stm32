...
void Read_IICData1(unsigned char addr, unsigned int *Data)
{
	...
	*Data = IIC_Read_Byte(0);       //读取一个字节数字量数据+非应答
  IIC_Stop();		//结束
}
/*********************************************************************
 *  函数名称：Read_IICData2
...
void Set_Mode(unsigned char addr, unsigned char new_data)
{
	IIC_Start(); // 开始
	IIC_Write_Byte(0x4c); // 写操作
	IIC_Write_Byte(addr); // 寄存器地址
	IIC_Write_Byte(new_data);// 写入新地址数据
	IIC_Stop();// 结束
}
